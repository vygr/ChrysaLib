...
void GUI_Task::add_front(std::shared_ptr<View> view)
{
	...
	auto msg_body = (GUI_Service::Event_add_front*)msg->begin();
	msg->set_dest(service_id);
	msg_body->m_evt = GUI_Service::evt_add_front;
	msg_body->m_reply = reply_id;
	msg_body->m_view = view;
	m_router.send(msg);
	//wait for reply
	reply_mbox->read();
...
void GUI_Task::add_back(std::shared_ptr<View> view)
{
	...
	auto msg_body = (GUI_Service::Event_add_back*)msg->begin();
	msg->set_dest(service_id);
	msg_body->m_evt = GUI_Service::evt_add_back;
	msg_body->m_reply = reply_id;
	msg_body->m_view = view;
	m_router.send(msg);
	//wait for reply
...
void GUI_Task::sub(std::shared_ptr<View> view)
{
	...
	auto msg_body = (GUI_Service::Event_sub*)msg->begin();
	msg->set_dest(service_id);
	msg_body->m_evt = GUI_Service::evt_sub;
	msg_body->m_reply = reply_id;
	msg_body->m_view = view;
	m_router.send(msg);
	//wait for reply
