...
//a service entry is of the format: "service_name,node_id:mbox_id,..."
...
{
public:
	//constructed with some crypto true random Node_ID !!!
	Router()
		: m_node_id(Node_ID::alloc())
	{
	...
	//get router node id
	auto const &get_node_id() const { return m_node_id; }
	...
	std::vector<std::string> enquire(const Node_ID &id, const std::string &prefix);
	...
	void add_link(Link *link, const Node_ID &id);
	void sub_link(Link *link);
	std::vector<Node_ID> get_peers();
	//routing management
	std::shared_ptr<Msg> get_next_msg(const Node_ID &dest, std::chrono::milliseconds timeout = std::chrono::milliseconds(0));
...
private:
	...
	std::map<Node_ID, Directory> m_directory;
